...
| Team pairings | `GET /t/:id/print/team-pairings?round=n` | Yes | Team round robin only: one table per match ("Match n: Team A - Team B", with the game-point score once results are in), a line per board with the first team's colour, both players, ratings and result; a team with the round's bye is listed as not playing. `round` defaults to `1`. 404s if round `n` has no team matches. See [`team-tournaments.md`](team-tournaments.md). |
| Team standings | `GET /t/:id/print/team-standings?round=n` | Yes (optional) | Team round robin only: rank, team, matches played, won-drawn-lost, match points and the configured team tie-breaks, after round `n` or current. |
