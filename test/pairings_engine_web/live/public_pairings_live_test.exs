defmodule PairingsEngineWeb.PublicPairingsLiveTest do
  ...
  test "renders a round's pairings sorted by board number regardless of insertion order", %{conn: conn} do
    {:ok, tournament} =
      Tournaments.create_tournament(%{"name" => "Board Order Test", "type" => "swiss", "rounds_count" => "1"})
  ...
  test "shows byes-table rows (SWAR-imported/round-specific absentee byes) alongside the round's pairings", %{
    ...
    {:ok, tournament} =
      Tournaments.create_tournament(%{"name" => "Byes Display Test", "type" => "swiss", "rounds_count" => "1"})
