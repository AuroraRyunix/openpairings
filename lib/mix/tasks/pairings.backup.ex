defmodule Mix.Tasks.Pairings.Backup do
  ...
  Backups, from the command line.
      ...
      mix pairings.backup --verify F   # open F and say what is in it
  ...
  @impl Mix.Task
  def run(argv) do
    {:ok, _} = Application.ensure_all_started(:pairings_engine)
...
")
        Mix.shell().info("  systemctl stop pairingsengine")
        Mix.shell().info("  mv #{live()} #{live()}.before-restore")
        Mix.shell().info("  mv #{target} #{live()}")
        Mix.shell().info("  systemctl start pairingsengine
")
