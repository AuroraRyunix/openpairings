defmodule PairingsEngine.Federations.BEL.SwarPublish do
  ...
  # Reads as what it is: OpenPairings, its real version, and the SWAR version
  # it is compatible with.
  #
  # The order is forced by the federation's list, which takes everything after
  # the LAST hyphen and shows the first token of it in its "Vers." column.
  # Two observations pin that rule: SWAR's `(C)FRBE-KBSB-v7.05 <date>` shows
  # as `v7.05`, and a bare `v7.00` showed as `7.00` - no hyphen at all, and
  # PHP's strrpos returns false, so substr($s, false + 1) chops exactly one
  # character.
  #
  # So the compatibility marker has to come LAST or the column shows the
  # wrong thing, while the file itself still reads honestly to anyone who
  # opens it. `OpenPairings-v7.00` was worse than either: it says this program
  # is on version 7.
  @default_version "OpenPairings #{PairingsEngine.Build.version()} -v7.00"
  ...
  # The footer strip a SWAR file carries, saying what wrote the page.
  #
  # Same three-cell shape and the same `tableCopyright` class, so it sits
  # where a reader of these files expects it - but it names this program and
  # nothing else. The federation's software puts its own name and its
  # author's initials here; copying either onto a page it had no hand in
  # would be claiming somebody else's work, the same reason the `Author` meta
  ...
  defp round_table(tournament, round) do
    ...
    rows =
      round.pairings
      # Played boards first, in board order, then everything with an empty
      # seat. A real published round puts its bye AFTER board 7, not between
      # boards 3 and 4 - which is what sorting on the board number alone did,
      # because a bye still carries one. The board cell is blank on those
      # rows, so a bye in the middle of the list reads as a gap in the
      # numbering rather than as the end of the boards.
      |> Enum.sort_by(fn p ->
        seated_both? = not is_nil(p.white_player_id) and not is_nil(p.black_player_id)
  ...
  # The awarded value in a bye or absence row, which SWAR prints WITHOUT the
  # decimal the points column carries: a real file shows `1` and `0` in that
  # cell beside `0.0` and `1.0` in the points cell. A half-point bye keeps the
  # same half symbol the result tokens use, rather than "0.5".
  defp fmt_award(value) when is_float(value) do
    cond do
      value == Float.round(value) -> value |> trunc() |> Integer.to_string()
      value == 0.5 -> "½"
      true -> Float.to_string(value)
    end
  end

  defp fmt_award(value) when is_integer(value), do: Integer.to_string(value)
  defp fmt_award(_), do: "0"
