defmodule PairingsEngine.Federations.BEL.SwarPublish do
  ...
  # SWAR's own shape, not a bare version number.
  #
  # The federation's public list shows a "Vers." column, and it does not print
  # this string as given: SWAR sends `(©)FRBE-KBSB-v7.05 2026/08/13` and the
  # column shows `v7.05`, while a bare `v7.00` came out as `7.00` - the
  # leading character eaten. Matching the shape SWAR sends is what makes the
  # column read the same as every other row.
  @default_version "(©)FRBE-KBSB-v7.00"
  ...
  #
  # Same three-cell shape and the same `tableCopyright` class, so it sits in
  # the page where a reader of these files expects it - but it says what is
  # true. SWAR's reads `(C)FRBE-KBSB-v7.05 <date>`, `(C) GMA 2015 - 2026` and
  # a build stamp; putting the federation's name and its author's initials on
  # a page this program produced would be claiming somebody else's work, the
  # same reason the `Author` meta is not copied either.
  defp colophon do
    """
    <table class='tableCopyright'>
    <tr><td style='text-align: center;'>#{esc(version())}</td>
        <td style='text-align: center;'>#{esc(gettext("Generated by OpenPairings"))}</td>
